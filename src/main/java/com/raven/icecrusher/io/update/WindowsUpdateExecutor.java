...
public class WindowsUpdateExecutor extends AbstractUpdateExecutor implements UpdateExecutor {

    private static final String LOCAL_INSTRUCTION_FILE = "update.bat";

    //keep the reference actively in memory to prevent the GC from
    //collecting it while downloading large update packages
    private NetworkService service;

    protected WindowsUpdateExecutor(final UpdateInfo updateInfo,
    ...
    public void downloadPackage(ProgressIndicator indicator, Label label){
        ...
        this.service = NetworkService.getService(runtimeChanged 
                ? ResourceLocator.UPDATE_PACKAGE_WINDOWS_FULL 
                        : ResourceLocator.UPDATE_PACKAGE_WINDOWS_APP);
    ...
    public void verifyDownload(){
        ...
        if((hash != null) && (!hash.isEmpty())){
            final String checksum = getUpdateInfo().getPackageChecksum(OperatingSystem.WINDOWS, 
    ...
    public void setupInstructions(){
        final Instruction instruct = super.getUpdateInfo()
                .getInstruction(OperatingSystem.WINDOWS);
        ...
        if(instruct != null){
            if(instruct.getType() == InstructionType.COPY_LOCAL){
                ...
                if(instruct.getID() != Const.UPDATE_WINDOWS_INSTRUCTION_ID){
    ...
    public void doUpdate(){
        ...
        final String argSource = ("\"" + path.substring(0, path.length()-1) + "\"");
        final String argTarget = ("\"" + targetDir.getAbsolutePath() + "\"\"");
        try{
            final ProcessBuilder proc = new ProcessBuilder("cmd.exe", "/C",
                    execution, argSource, argTarget);
    ...
    private void copyRemoteInstructions(final Instruction instruct){
        this.service = NetworkService.getService(ResourceLocator.UPDATE_INSTRUCT_WINDOWS);
        this.service.setOnResult((result) -> {
            ...
            if(isOK){
                ...
                super.copyRemoteInstructions(result.getBytes(),
                        LOCAL_INSTRUCTION_FILE, (success) -> {
