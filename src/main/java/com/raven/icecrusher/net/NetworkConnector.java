...
public class NetworkConnector extends Task<NetworkResult> {
	...
	/**
	 * Opens a HttpURLConnection/HttpsURLConnection on the URL object specified by the internal 
	 * resource locator
	 * 
	...
	private void sendPayload() throws IOException{
		try(final BufferedOutputStream os = new BufferedOutputStream(connection.getOutputStream())){
	...
	private Header receiveResponseHeader(){
		...
		for(final Map.Entry<String, List<String>> field : connection.getHeaderFields().entrySet()){
	...
	/**
	 * Constructs and returns a NetworkResult object for the response of the underlying connection
	 * 
		...
		try{
			...
			while((i = is.read(buffer, 0, buffer.length)) != -1){
				...
				if(contentLength >= 0){
					...
					updateMessage(String.valueOf((int)(((float)size/(float)contentLength)*100)) + "%");
				}
			}
		}finally{
			is.close();
