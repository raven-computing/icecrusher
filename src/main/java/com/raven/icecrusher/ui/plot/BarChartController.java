...
public class BarChartController extends ChartController {
	...
	private static final ObservableList<String> OPTIONS_Y_MODE = FXCollections.observableArrayList(
	 ...
	 */
	private void updateBarColor(final XYChart.Series<String, Number> series, final String color){
        for(final XYChart.Data<String, Number> data : series.getData()){
	...
	private void prepareChart(){
		...
		if((chart.getData().size() + preparedSeries.size()+1) > 8){
			OneShotSnackbar.showFor(getRootNode(), "The maximum number of series has been reached");
			return ;
		}
	...
	private Series<String, Number> prepareData(final Column colX, final Column colY){
		final String yColName = df.getColumnName(cbColumnY.getSelectionModel().getSelectedIndex());
		...
		try{
			map = createMapping(colX, colY, cbYMode.getSelectionModel().getSelectedIndex());
	...
	private Map<Object, Double> createMapping(final Column colX, final Column colY, final int opMode)
	...
	private void addHoverListenerFor(final XYChart.Data<String, Number> data){
		...
		data.getNode().hoverProperty().addListener((ov, oldValue, newValue) -> {
			...
			Platform.runLater(() -> {
				...
				text.setFill((checkWhiteBackground.isSelected() ? Color.BLACK : Color.WHITE));
