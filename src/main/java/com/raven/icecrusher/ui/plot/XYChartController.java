...
public abstract class XYChartController extends ChartController {
	...
	protected static final ObservableList<String> OPTIONS_DATE_FORMAT = 
			...
			"yyyy-MM-dd", "yyyyMMdd", "yyyy.MM.dd", "dd-MM-yyyy", "ddMMyyyy", "dd.MM.yyyy");
	 ...
	 * 
	 * @param chart The <code>XYChart</code> instance used by the concrete chart implementation
	 */
	protected void initialize(final XYChart<Number, Number> chart){
		...
		this.checkXIsDate.selectedProperty().addListener((ov, oldValue, newValue) -> {
		...
		this.checkWhiteBackground.selectedProperty().addListener((ov, oldValue, newValue) -> {
	...
	public void onStart(ArgumentBundle bundle){
		...
		this.cbColumnX.getSelectionModel().selectedItemProperty().addListener((ov, oldValue, newValue) -> {
			columnSelectionChanged();
			reset();
		});
		this.cbColumnY.getItems().addAll(df.getColumnNames());
		this.cbColumnY.getSelectionModel().selectedItemProperty().addListener((ov, oldValue, newValue) -> {
			columnSelectionChanged();
		});
	 ...
	 * 
	 * @return A <code>DateTimeFormatter</code> for parsing dates according to the selected format
	 */
	...
	/**
	 * Returns the column object that corresponds to the selected column in the x-axis combo box
	 * 
	...
	/**
	 * Returns the column object that corresponds to the selected column in the y-axis combo box
	 * 
	 * @return The <code>Column</code> selected by the user
	 */
	...
	protected void updateSymbolsColor(final ObservableList<Data<Number, Number>> data, final String color){
	...
	protected XYChartData prepareData(final Column colX, final Column colY){
		...
		if(DataFrames.columnUsesNaNs(colX) && !DataFrames.columnUsesStrings(colX)){
			OneShotSnackbar.showFor(getRootNode(), "Only numbers and strings are allowed for the x-Axis");
			return null;
		}
		...
		if(usedDataList.contains(colY)){
			OneShotSnackbar.showFor(getRootNode(), "This data series has already been added");
			return null;
		}
		...
		if(xAxisIsDate){//X-AXIS IS DATE FORMAT
			if(cbDateFormat.getSelectionModel().getSelectedIndex() == -1){
				OneShotSnackbar.showFor(getRootNode(), "Please select a date format");
				return null;
			}
			try{
				data = prepareTimeData(colX, colY);
			}catch(DateFormatException ex){
				OneShotSnackbar.showFor(getRootNode(), "Incorrectly formatted date at index " 
							+ ex.getCauseIndex());
				
				return null;
			}catch(DataFrameException ex){
				OneShotSnackbar.showFor(getRootNode(), "No valid data points for selected columns");
				return null;
			}
		}else{//X-AXIS IS NUMERIC
			if(DataFrames.columnUsesStrings(colX)){
				OneShotSnackbar.showFor(getRootNode(), "Strings must represent dates");
				return null;
			}
	...
	private XYChartData prepareTimeData(final Column colX, final Column colY) 
		...
		final LocalDate tZero = LocalDate.parse(String.valueOf(dates.getInt(0, 0)), DATE_FORMATTER_ENCODED);
		min = yZero.doubleValue();
		max = yZero.doubleValue();
		
		for(int i=1; i<dates.rows(); ++i){
			final LocalDate d = LocalDate.parse(String.valueOf(dates.getInt(0, i)), DATE_FORMATTER_ENCODED);
		...
		final Period p = Period.between(tZero, 
				LocalDate.parse(String.valueOf(dates.getInt(0, dates.rows()-1)), DATE_FORMATTER_ENCODED));
	...
	private DataFrame buildSortedXYMapping(final Column colX, final Column colY) throws DateFormatException{
	...
	private DataFrame buildNonNullDataFrame(final Integer[] rawX, final Column colY, final int validDataPoints){
