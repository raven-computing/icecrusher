		...
		this.service.setOnResult((result) -> {
			...
			if(result.getSatus() == Status.SUCCESS){
				if((result.getResponseCode() == HttpURLConnection.HTTP_OK) 
					...
					OneShotSnackbar.showFor(getRootNode(), "Thank you for your feedback!");
					finishActivity();
				}else if(result.getResponseCode() == HttpURLConnection.HTTP_BAD_REQUEST){
					btnSend.setText("Send");
					OneShotSnackbar.showFor(getRootNode(), "Your feedback contains invalid characters");
				}else if(result.getResponseCode() == HttpURLConnection.HTTP_ENTITY_TOO_LARGE){
					btnSend.setText("Send");
					OneShotSnackbar.showFor(getRootNode(), "The feedback text or E-Mail you entered is too long");
				}else{
